...
def fetch(repo: str) -> list[GitBranch]:
    ...
    logger.debug(f"[BEGIN] Fetching branches for {repo}")

    branches = data_cache.get(f"Git Branches {repo}")
    if branches:
        logging.debug(f"Found branches for {repo} in cache")
        return branches
    ...
    if response.status_code == 200:
        ...
        logger.debug(f"Cache branches for {repo}.")
        data_cache.push(key=f"Git Branches {repo}", value=branches)

        logger.debug(f"[END] Fetching branches for {repo}")
        return branches

    logger.error(f"Error fetching branches for {repo}: {response.text}")
